...
class RGCNConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, edge_type_ptr: OptTensor) -> Tensor:
        if torch_geometric.typing.WITH_PYG_LIB and edge_type_ptr is not None:
            ...
            # print("Inside matmul with triton")
            ...
            types = torch.zeros(x_j.shape[0], dtype=torch.long, device="cuda", requires_grad=False)
            for i, s in enumerate(edge_ptr):
                types[s] = i
            sorted_data, tensor_slice = compact_tensor_types(x_j, types, device="cuda")
            output = ops.fasten_segment_matmul(sorted_data, tensor_slice.slices, self.weight, Engine.TRITON, tensor_slice)
            assert(torch_output.shape == output.shape)
            print("Max difference:", torch.max(torch.abs(output-torch_output)))
            return output
