...
def test_correctness():
    ...
    fasten_rgcn_conv = FastenRGCNConv(4, 32, 8, is_sorted=True, aggr='add').to(device)
...
def test_benchmark():
    ...
    tensor_slice = compact_tensor_types(types=edge_type, data=edge_index, dim=1, device=device)
    sorted_edge_type = slices_to_tensor(tensor_slice)
    rgcn_conv = RGCNConv(num_features, num_features, num_types, is_sorted=True, aggr='add').to(device)
    ms = triton.testing.do_bench(lambda: rgcn_conv(x, tensor_slice.data, sorted_edge_type))
    print("Torch time:", ms)

    fasten_rgcn_conv = FastenRGCNConv(num_features, num_features, num_types, is_sorted=True, aggr='add').to(device)
